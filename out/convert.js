...
const camelcase = (x) => x.replace(/-[a-z]/g, (c) => c[1].toUpperCase());
const indent = (x) => x.split("\n").join("\n  ");
function stringify(obj, isTop) {
...
exports.default = postcss.plugin("postcss-css-to-js", (options = {}) => {
    return (css) => {
        const lines = [];
        ...
        css.walkRules((rule) => {
            ...
            namespaces.forEach((item) => {
                ...
                rule.walkDecls((decl) => {
                    ...
                    if (unitlessNumberProperties.has(decl.prop) &&
                        value.match(/^[0-9.]+(px)?$/)) {
